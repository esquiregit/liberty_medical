<?php
	...
	if($request) {
		$result = BueCreatinineEgfr::create_bue_creatinine_egfr($patient_id, $sodium, $sodium_flag, $potassium, $potassium_flag, $chloride, $chloride_flag, $urea, $urea_flag, $creatinine, $creatinine_flag, $egfr, $comments, $added_by, $conn);
		if($result) {
			array_push($response, array(
				"status"  => "Success",
				"message" => "BUE Creatinine eGFR Lab Added Successfully...."
			));
			Audit_Trail::create_log($added_by, 'Added Bue Creatinine eGFR Lab For '.$patient, $conn);
		} else {
			array_push($response, array(
				"status"  => "Failed",
				"message" => "BUE Creatinine eGFR Lab Could Not Be Added. Please Try Again...."
			));
			Audit_Trail::create_log($added_by, 'Tried To Add Bue Creatinine eGFR Lab For '.$patient, $conn);
		}
	}
