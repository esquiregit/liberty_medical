<?php
	...
	if($request) {
		...
		$result  = BueCreatinineEgfr::update_bue_creatinine_egfr($id, $patient_id, $sodium, $sodium_flag, $potassium, $potassium_flag, $chloride, $chloride_flag, $urea, $urea_flag, $creatinine, $creatinine_flag, $egfr, $comments, $conn);
		if($result) {
			array_push($response, array(
				"status"  => "Success",
				"message" => "BUE Creatinine eGFR Lab Updated Successfully...."
			));
			Audit_Trail::create_log($added_by, 'Updated Bue Creatinine eGFR Lab For '.$patient, $conn);
		} else {
			array_push($response, array(
				"status"  => "Failed",
				"message" => "BUE Creatinine eGFR Lab Could Not Be Updated. Please Try Again...."
			));
			Audit_Trail::create_log($added_by, 'Tried To Update Bue Creatinine eGFR Lab For '.$patient, $conn);
		}
	}
