...
function AddEyeSwabCS({ patient, closeModal }) {
    ...
    const handleClose  = () => { setOpen(false); closeModal('EyeSwabCS'); };
    ...
    const onSubmit     = () => {
        ...
        Axios.post(getBaseURL()+'add_eye_swab', values, { signal: signal })
            .then(response => {
                if(response.data[0].status.toLowerCase() === 'success') {
                    setSuccess(true);
                    setMessage(response.data[0].message);
                    setTimeout(() => { closeModal('EyeSwabCS'); }, 1050);
                } else {
                    setError(true);
                    setMessage(response.data[0].message);
                }
