...
function AddHVSCS({ patient, closeModal }) {
    ...
    const handleClose  = () => { setOpen(false); closeModal('HVSCS'); };
    ...
    const onSubmit     = () => {
        ...
        Axios.post(getBaseURL()+'add_hvs_cs', values, { signal: signal })
            .then(response => {
                if(response.data[0].status.toLowerCase() === 'success') {
                    setSuccess(true);
                    setMessage(response.data[0].message);
                    setTimeout(() => { closeModal('HVSCS'); }, 1050);
                } else {
                    setError(true);
                    setMessage(response.data[0].message);
                }
