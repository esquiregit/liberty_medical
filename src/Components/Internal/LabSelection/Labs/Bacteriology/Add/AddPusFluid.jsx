...
import { getTodaysDate, getAntibiotics, getAppearances, getBacterias, getGramStains, getCultures, getColours } from '../../../../../Extras/Functions';
...
function AddPusFluid({ patient, closeModal }) {
    ...
    const handleClose  = () => { setOpen(false); closeModal('PusFluid'); };
    ...
    const onSubmit     = () => {
        ...
        Axios.post(getBaseURL()+'add_pus_fluid', values, { signal: signal })
            .then(response => {
                if(response.data[0].status.toLowerCase() === 'success') {
                    setSuccess(true);
                    setMessage(response.data[0].message);
                    setTimeout(() => { closeModal('PusFluid'); }, 1050);
                } else {
                    setError(true);
                    setMessage(response.data[0].message);
                }
