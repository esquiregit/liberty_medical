...
function AddEarSwabCS({ patient, closeModal }) {
    ...
    const handleClose  = () => { setOpen(false); closeModal('EarSwabCS'); };
    ...
    const onSubmit     = () => {
        ...
        Axios.post(getBaseURL()+'add_ear_swab', values, { signal: signal })
            .then(response => {
                if(response.data[0].status.toLowerCase() === 'success') {
                    setSuccess(true);
                    setMessage(response.data[0].message);
                    setTimeout(() => { closeModal('EarSwabCS'); }, 1050);
                } else {
                    setError(true);
                    setMessage(response.data[0].message);
                }
