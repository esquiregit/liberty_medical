...
function AddSemenCS({ patient, closeModal }) {
    ...
    const handleClose  = () => { setOpen(false); closeModal('SemenCS'); };
    ...
    const onSubmit     = () => {
        ...
        Axios.post(getBaseURL()+'add_semen_cs', values, { signal: signal })
            .then(response => {
                if(response.data[0].status.toLowerCase() === 'success') {
                    setSuccess(true);
                    setMessage(response.data[0].message);
                    setTimeout(() => { closeModal('SemenCS'); }, 1050);
                } else {
                    setError(true);
                    setMessage(response.data[0].message);
                }
