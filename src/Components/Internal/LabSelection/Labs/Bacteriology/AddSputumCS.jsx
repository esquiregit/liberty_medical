...
function AddSputumCS({ patient, closeModal }) {
    ...
    const onSubmit     = () => {
        ...
        Axios.post(getBaseURL()+'add_sputum_cs', values, { signal: signal })
            .then(response => {
                if(response.data[0].status.toLowerCase() === 'success') {
                    setSuccess(true);
                    setMessage(response.data[0].message);
                    setTimeout(() => { closeModal('sputumcs'); }, 1050);
                } else {
                    setError(true);
                    setMessage(response.data[0].message);
                }
