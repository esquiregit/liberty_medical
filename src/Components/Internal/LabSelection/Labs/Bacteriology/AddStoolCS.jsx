...
function AddStoolCS({ patient, closeModal }) {
    ...
    const handleClose  = () => { setOpen(false); closeModal('StoolCS'); };
    ...
    const onSubmit     = () => {
        ...
        Axios.post(getBaseURL()+'add_stool_cs', values, { signal: signal })
            .then(response => {
                if(response.data[0].status.toLowerCase() === 'success') {
                    setSuccess(true);
                    setMessage(response.data[0].message);
                    setTimeout(() => { closeModal('StoolCS'); }, 1050);
                } else {
                    setError(true);
                    setMessage(response.data[0].message);
                }
