...
function AddThroatSwabCS({ patient, closeModal }) {
    ...
    const handleClose  = () => { setOpen(false); closeModal('ThroatSwabCS'); };
    ...
    const onSubmit     = () => {
        ...
        Axios.post(getBaseURL()+'add_throat_swab', values, { signal: signal })
            .then(response => {
                if(response.data[0].status.toLowerCase() === 'success') {
                    setSuccess(true);
                    setMessage(response.data[0].message);
                    setTimeout(() => { closeModal('ThroatSwabCS'); }, 1050);
                } else {
                    setError(true);
                    setMessage(response.data[0].message);
                }
...
export default AddThroatSwabCS;
