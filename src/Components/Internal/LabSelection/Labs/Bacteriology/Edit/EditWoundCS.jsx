...
import { getTodaysDate, getAntibiotics, getGramStains, getBacterias, getZNStains, getCultures, get_pus_cells_options, getFungalElements } from '../../../../../Extras/Functions';
...
function EditWoundCS({ lab, closeModal, closeExpandable }) {
    ...
    const handleClose  = () => { setOpen(false); closeModal('woundcs'); };
    ...
    const onSubmit     = () => {
        ...
        Axios.post(getBaseURL()+'edit_wound_cs', values, { signal: signal })
            .then(response => {
                if(response.data[0].status.toLowerCase() === 'success') {
                    setSuccess(true);
                    setMessage(response.data[0].message);
                    setTimeout(() => { closeModal('woundcs'); }, 1050);
                } else {
                    setError(true);
                    setMessage(response.data[0].message);
                }
                            ...
                            <DialogContent dividers>
                                ...
                                <table id="lab-table">
                                    <tbody>
                                        ...
                                        <tr>
                                            <td></td>
                                            <td width="25%" className="text-left pr-10">Fungal Element</td>
                                            <td width="25%">
                                                <FormikTextField
                                                    select
                                                    size="small"
                                                    ...
                                                    id="fungal_element"
                                                    name="fungal_element">
                                                    {getFungalElements().map((option, index) => (
                                                        <MenuItem key={index} value={option.value}>
                                                            {option.label}
                                                        </MenuItem>
                                                    ))}
