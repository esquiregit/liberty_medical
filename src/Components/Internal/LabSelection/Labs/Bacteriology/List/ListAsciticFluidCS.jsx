...
import ViewAsciticFluidCS from '../View/ViewAsciticFluid';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListAsciticFluidCS({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Ascitic Fluid CS | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_ascitic_fluid_cs', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewAsciticFluidCS
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Ascitic Fluid CS" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Ascitic Fluid CS Lab" plural="Ascitic Fluid CS Labs" />
                }
            </main>
            <Footer />
        </>
...
export default ListAsciticFluidCS;
