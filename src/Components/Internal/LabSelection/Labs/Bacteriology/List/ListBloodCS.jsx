...
import ViewBloodCS from '../View/ViewBloodCS';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListBloodCS({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Blood CS | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_blood_cs', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewBloodCS
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Blood CS" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Blood CS Lab" plural="Blood CS Labs" />
                }
            </main>
            <Footer />
        </>
