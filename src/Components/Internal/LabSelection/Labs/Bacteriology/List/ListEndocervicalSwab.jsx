...
import ViewEndocervicalSwab from '../View/ViewEndocervicalSwab';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListEndocervicalSwab({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Endocervical Swab | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_endocervical_swab', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewEndocervicalSwab
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Endocervical Swab" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Endocervical Swab Lab" plural="Endocervical Swab Labs" />
                }
            </main>
            <Footer />
        </>
...
export default ListEndocervicalSwab;
