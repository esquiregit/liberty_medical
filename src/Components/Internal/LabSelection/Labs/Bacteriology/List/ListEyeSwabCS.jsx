...
import ViewEyeSwabCS from '../View/ViewEyeSwabCS';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListEyeSwabCS({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Eye Swab CS | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_eye_swab_cs', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewEyeSwabCS
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Eye Swab CS" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Eye Swab CS Lab" plural="Eye Swab CS Labs" />
                }
            </main>
            <Footer />
        </>
