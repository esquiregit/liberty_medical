...
import ViewPleuralFluid from '../View/ViewPleuralFluid';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListPleuralFluid({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Pleural Fluid | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_pleural_fluid', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewPleuralFluid
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Pleural Fluid" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Pleural Fluid Lab" plural="Pleural Fluid Labs" />
                }
            </main>
            <Footer />
        </>
...
export default ListPleuralFluid;
