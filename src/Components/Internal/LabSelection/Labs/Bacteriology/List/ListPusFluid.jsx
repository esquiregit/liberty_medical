...
import ViewPusFluidCS from '../View/ViewPusFluid';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListPusFluidCS({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Pus Fluid | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_pus_fluid', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewPusFluidCS
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Pus Fluid" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Pus Fluid Lab" plural="Pus Fluid Labs" />
                }
            </main>
            <Footer />
        </>
