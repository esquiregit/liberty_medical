...
import ViewSkinSnip from '../View/ViewSkinSnip';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListSkinSnip({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Skin Snip | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_skin_snip', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const columns   = [
        ...
        },
        {
            ...
            name: "onchocerca_volvulus",
            options: {
                filter: true,
            }
        },
        {
            label: "M. Streptocerca",
            name: "m_streptocerca",
            options: {
                filter: true,
            }
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewSkinSnip
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Skin Snip" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Skin Snip Lab" plural="Skin Snip Labs" />
                }
            </main>
            <Footer />
        </>
