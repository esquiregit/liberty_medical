...
import ViewStoolCS from '../View/ViewStoolCS';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListStoolCS({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Stool CS | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_stool_cs', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewStoolCS
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Stool CS" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Stool CS Lab" plural="Stool CS Labs" />
                }
            </main>
            <Footer />
        </>
