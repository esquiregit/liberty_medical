...
import ViewStoolRE from '../View/ViewStoolRE';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListStoolRE({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Stool RE | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_stool_re', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewStoolRE
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Stool RE" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Stool RE Lab" plural="Stool RE Labs" />
                }
            </main>
            <Footer />
        </>
