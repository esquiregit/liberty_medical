...
import ViewUrineCS from '../View/ViewUrineCS';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListUrineCS({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Urine CS | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_urine_cs', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewUrineCS
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Urine CS" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Urine CS Lab" plural="Urine CS Labs" />
                }
            </main>
            <Footer />
        </>
