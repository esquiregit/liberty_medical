...
import ViewUrineRE from '../View/ViewUrineRE';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListUrineRE({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Urine RE | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_urine_re', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewUrineRE
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Urine RE" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Urine RE Lab" plural="Urine RE Labs" />
                }
            </main>
            <Footer />
        </>
