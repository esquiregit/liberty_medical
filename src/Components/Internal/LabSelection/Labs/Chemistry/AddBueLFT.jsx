...
function AddBueLFT({ patient, closeModal }) {
    ...
    const onSubmit     = () => {
        ...
        Axios.post(getBaseURL()+'add_bue_creatinine_lft', values, { signal: signal })
            .then(response => {
                if(response.data[0].status.toLowerCase() === 'success') {
                    setSuccess(true);
                    setMessage(response.data[0].message);
                    setTimeout(() => { closeModal('buelft'); }, 1050);
                } else {
                    setError(true);
                    setMessage(response.data[0].message);
                }
