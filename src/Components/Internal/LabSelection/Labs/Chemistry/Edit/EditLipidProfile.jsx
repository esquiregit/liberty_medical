...
function EditLipidProfile({ lab, closeModal, closeExpandable }) {
    ...
    const handleClose  = () => { setOpen(false); closeModal('lipidProfile'); };
    ...
    const onSubmit     = () => {
        ...
        Axios.post(getBaseURL()+'edit_lipid_profile', values, { signal: signal })
            .then(response => {
                if(response.data[0].status.toLowerCase() === 'success') {
                    setSuccess(true);
                    setMessage(response.data[0].message);
                    setTimeout(() => { closeModal('lipidProfile'); }, 1050);
                } else {
                    setError(true);
                    setMessage(response.data[0].message);
                }
...
export default EditLipidProfile;
