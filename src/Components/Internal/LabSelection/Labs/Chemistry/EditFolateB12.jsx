...
function EditFolateB12({ lab, patient, closeModal }) {
    ...
    const handleClose  = () => { setOpen(false); closeModal('folateb12'); };
    ...
    const onSubmit     = () => {
        ...
        Axios.post(getBaseURL()+'edit_folate_b12', values, { signal: signal })
            .then(response => {
                if(response.data[0].status.toLowerCase() === 'success') {
                    setSuccess(true);
                    setMessage(response.data[0].message);
                    setTimeout(() => { closeModal('folateb12'); }, 1050);
                } else {
                    setError(true);
                    setMessage(response.data[0].message);
                }
