...
function Editserumlipase({ lab, patient, closeModal }) {
    ...
    const initialValues = {
        ...
        s_lipase_flag : lab.s_lipase_flag,
    ...
    const handleClose  = () => { setOpen(false); closeModal('serumlipase'); };
    ...
    const onSubmit     = () => {
        ...
        Axios.post(getBaseURL()+'edit_serum_lipase', values, { signal: signal })
            .then(response => {
                if(response.data[0].status.toLowerCase() === 'success') {
                    setSuccess(true);
                    setMessage(response.data[0].message);
                    setTimeout(() => { closeModal('serumlipase'); }, 1050);
                } else {
                    setError(true);
                    setMessage(response.data[0].message);
                }
...
export default Editserumlipase;
