...
import ViewBueCreatinine from '../View/ViewBueCreatinine';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListBueCreatinine({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Bue + Creatinine | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_bue_creatinine', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewBueCreatinine
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Bue + Creatinine" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Bue + Creatinine Lab" plural="Bue + Creatinine Labs" />
                }
            </main>
            <Footer />
        </>
...
export default ListBueCreatinine;
