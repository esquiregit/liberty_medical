...
import ViewBueCreatinineEgfr from '../View/ViewBueCReatinineEgfr';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListBueCreatinineEgfr({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Bue + Creatinine + eGFR | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_bue_creatinine_egfr', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewBueCreatinineEgfr
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Bue + Creatinine + eGFR" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Bue + Creatinine + eGFR Lab" plural="Bue + Creatinine + eGFR Labs" />
                }
            </main>
            <Footer />
        </>
...
export default ListBueCreatinineEgfr;
