...
import ViewBueLFT from '../View/ViewBueLFT';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListBueLFT({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Bue + LFT | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_bue_creatinine_lft', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewBueLFT
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Bue + Creatinine + LFT" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
