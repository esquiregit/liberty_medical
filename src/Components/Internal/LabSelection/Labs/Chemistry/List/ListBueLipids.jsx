...
import ViewBueLipids from '../View/ViewBueLipids';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListBueLipids({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Bue + Lipids | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_bue_creatinine_lipids', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewBueLipids
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Bue + Creatinine + Lipids" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Bue + Lipids Lab" plural="Bue + Lipids Labs" />
                }
            </main>
            <Footer />
        </>
