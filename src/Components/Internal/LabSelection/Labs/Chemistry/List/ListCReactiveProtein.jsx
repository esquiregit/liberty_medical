...
import ViewCReactiveProtein from '../View/ViewCReactiveProtein';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListCReactiveProtein({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'C-Reactive Protein | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_c_reactive_protein', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewCReactiveProtein
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / C-Reactive Protein" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="C-Reactive Protein Lab" plural="C-Reactive Protein Labs" />
                }
            </main>
            <Footer />
        </>
...
export default ListCReactiveProtein;
