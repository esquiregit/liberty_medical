...
import ViewCalciumProfile from '../View/ViewCalciumProfile';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListCalciumProfile({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Calcium Profile | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_calcium_profile', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewCalciumProfile
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Calcium Profile" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Calcium Profile Lab" plural="Calcium Profile Labs" />
                }
            </main>
            <Footer />
        </>
...
export default ListCalciumProfile;
