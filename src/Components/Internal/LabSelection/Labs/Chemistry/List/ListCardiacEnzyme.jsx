...
import ViewCardiacEnzyme from '../View/ViewCardiacEnzyme';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListCardiacEnzyme({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Cardiac Enzyme | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_cardiac_enzyme', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewCardiacEnzyme
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Cardiac Enzyme" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Cardiac Enzyme Lab" plural="Cardiac Enzyme Labs" />
                }
            </main>
            <Footer />
        </>
...
export default ListCardiacEnzyme;
