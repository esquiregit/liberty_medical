...
import ViewCompactChemistry from '../View/ViewCompactChemistry';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListCompactChemistry({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Compact Chemistry | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_compact_chemistry', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewCompactChemistry
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Compact Chemistry" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Compact Chemistry Lab" plural="Compact Chemistry Labs" />
                }
            </main>
            <Footer />
        </>
...
export default ListCompactChemistry;
