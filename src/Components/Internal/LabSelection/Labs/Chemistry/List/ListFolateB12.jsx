...
import ViewFolateB12 from '../View/ViewFolateB12';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListFolateB12({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Folate B12 | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_folate_b12', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewFolateB12
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Folate B12" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Folate B12 Lab" plural="Folate B12 Labs" />
                }
            </main>
            <Footer />
        </>
