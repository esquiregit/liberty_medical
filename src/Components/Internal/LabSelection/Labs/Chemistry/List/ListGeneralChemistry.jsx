...
import ViewGeneralChemistry from '../View/ViewGeneralChemistry';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListGeneralChemistry({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'General Chemistry | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_general_chemistry', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewGeneralChemistry
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / General Chemistry" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="General Chemistry Lab" plural="General Chemistry Labs" />
                }
            </main>
            <Footer />
        </>
...
export default ListGeneralChemistry;
