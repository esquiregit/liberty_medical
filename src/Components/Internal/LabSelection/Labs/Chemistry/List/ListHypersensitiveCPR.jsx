...
import ViewHypersensitiveCPR from '../View/ViewHypersensitiveCPR';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListHypersensitiveCPR({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Hypersensitive CPR | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_hypersensitive_cpr', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewHypersensitiveCPR
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Hyper Sensitive CPR" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Hyper Sensitive CPR Lab" plural="Hyper Sensitive CPR Labs" />
                }
            </main>
            <Footer />
        </>
...
export default ListHypersensitiveCPR;
