...
import ViewIronStudy from '../View/ViewIronStudy';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListIronStudy({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'IronStudy | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_iron_study', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewIronStudy
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Iron Study" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Iron Study Lab" plural="Iron Study Labs" />
                }
            </main>
            <Footer />
        </>
