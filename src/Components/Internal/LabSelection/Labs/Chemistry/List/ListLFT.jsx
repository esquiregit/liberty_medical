...
import ViewLFT from '../View/ViewLFT';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListLFT({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'LFT | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_lft', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewLFT
                                                        history={history}
                    ...
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ...
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="LFT Lab" plural="LFT Labs" />
                }
            </main>
            <Footer />
        </>
