...
import ViewLipidProfile from '../View/ViewLipidProfile';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListLipidProfile({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Lipids Profile | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_lipid_profile', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewLipidProfile
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Lipids Profile" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Lipids Profile Lab" plural="Lipids Profile Labs" />
                }
            </main>
            <Footer />
        </>
...
export default ListLipidProfile;
