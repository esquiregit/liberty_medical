...
import ViewProteinElectrophoresis from '../View/ViewProteinElectrophoresis';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListProteinElectrophoresis({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Protein Electrophoresis | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_protein_electrophoresis', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewProteinElectrophoresis
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Protein Electrophoresis" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Protein Electrophoresis Lab" plural="Protein Electrophoresis Labs" />
                }
            </main>
            <Footer />
        </>
...
export default ListProteinElectrophoresis;
