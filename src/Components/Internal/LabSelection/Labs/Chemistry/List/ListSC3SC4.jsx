...
import ViewSC3SC4 from '../View/ViewSC3SC4';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListSC3SC4({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'SC3 SC4 | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_sc3_sc4', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewSC3SC4
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / SC3 SC4" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="SC3 SC4 Lab" plural="SC3 SC4 Labs" />
                }
            </main>
            <Footer />
        </>
