...
import ViewSerumLipase from '../View/ViewSerumLipase';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListSerumLipase({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Serum Lipase | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_serum_lipase', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewSerumLipase
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Serum Lipase" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Serum Lipase Lab" plural="Serum Lipase Labs" />
                }
            </main>
            <Footer />
        </>
...
export default ListSerumLipase;
