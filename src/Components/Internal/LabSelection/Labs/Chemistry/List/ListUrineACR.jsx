...
import ViewUrineACR from '../View/ViewUrineACR';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListUrineACR({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Urine ACR | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_urine_acr', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewUrineACR
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Urine ACR" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Urine ACR Lab" plural="Urine ACR Labs" />
                }
            </main>
            <Footer />
        </>
