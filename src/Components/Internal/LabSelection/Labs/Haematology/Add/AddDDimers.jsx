...
function AddDDimers({ patient, closeModal }) {
    ...
    const onSubmit     = () => {
        ...
        Axios.post(getBaseURL()+'add_d_dimers', values, { signal: signal })
            .then(response => {
                if(response.data[0].status.toLowerCase() === 'success') {
                    setSuccess(true);
                    setMessage(response.data[0].message);
                    setTimeout(() => { closeModal('DDimers'); }, 1050);
                } else {
                    setError(true);
                    setMessage(response.data[0].message);
                }
