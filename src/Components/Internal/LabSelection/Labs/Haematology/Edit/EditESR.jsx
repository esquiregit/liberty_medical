...
function EditESR({ lab, closeModal, closeExpandable }) {
    ...
    const handleClose  = () => { setOpen(false); closeModal('ESR'); };
    ...
    const onSubmit     = () => {
        ...
        Axios.post(getBaseURL()+'edit_esr', values, { signal: signal })
            .then(response => {
                if(response.data[0].status.toLowerCase() === 'success') {
                    setSuccess(true);
                    setMessage(response.data[0].message);
                    setTimeout(() => { closeModal('ESR'); }, 1050);
                } else {
                    setError(true);
                    setMessage(response.data[0].message);
                }
