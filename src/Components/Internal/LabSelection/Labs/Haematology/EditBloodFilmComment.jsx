...
function EditBloodFilmComment({ lab, patient, closeModal }) {
    ...
    const handleClose  = () => { setOpen(false); closeModal('BloodFilmComment'); };
    ...
    const onSubmit     = () => {
        ...
        Axios.post(getBaseURL()+'add_blood_film_comment', values, { signal: signal })
            .then(response => {
                if(response.data[0].status.toLowerCase() === 'success') {
                    setSuccess(true);
                    setMessage(response.data[0].message);
                    setTimeout(() => { closeModal('BloodFilmComment'); }, 1050);
                } else {
                    setError(true);
                    setMessage(response.data[0].message);
                }
...
export default EditBloodFilmComment;
