...
import ViewBloodFilmComment from '../View/ViewBloodFilmComment';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListBloodFilmComment({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Blood Film Comment | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_blood_film_comment', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewBloodFilmComment
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Blood Film Comment" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Blood Film Comment Lab" plural="Blood Film Comment Labs" />
                }
            </main>
            <Footer />
        </>
...
export default ListBloodFilmComment;
