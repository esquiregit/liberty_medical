...
import ViewDDimers from '../View/ViewDDimers';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListDDimers({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'D-Dimers | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_d_dimers', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewDDimers
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / D-Dimers" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="D-Dimers Lab" plural="D-Dimers Labs" />
                }
            </main>
            <Footer />
        </>
