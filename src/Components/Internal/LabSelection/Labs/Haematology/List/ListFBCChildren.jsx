...
import ViewFBCChildren from '../View/ViewFBCChildren';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListFBCChildren({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'FBC Children | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_fbc_children', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewFBCChildren
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / FBC Children" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="FBC Children Lab" plural="FBC Children Labs" />
                }
            </main>
            <Footer />
        </>
...
export default ListFBCChildren;
