...
import ViewNTCScreening from '../View/ViewNTCScreening';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListNTCScreening({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'NTC Screening | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_ntc_screening', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewNTCScreening
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / NTC Screening" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="NTC Screening Lab" plural="NTC Screening Labs" />
                }
            </main>
            <Footer />
        </>
...
export default ListNTCScreening;
