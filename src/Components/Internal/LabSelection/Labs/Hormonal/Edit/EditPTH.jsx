...
function EditPTH({ lab, closeModal, closeExpandable }) {
    ...
    const handleClose  = () => { setOpen(false); closeModal('pth'); };
    ...
    const onSubmit     = () => {
        ...
        Axios.post(getBaseURL()+'edit_pth', values, { signal: signal })
            .then(response => {
                if(response.data[0].status.toLowerCase() === 'success') {
                    setSuccess(true);
                    setMessage(response.data[0].message);
                    setTimeout(() => { closeModal('pth'); }, 1050);
                } else {
                    setError(true);
                    setMessage(response.data[0].message);
                }
