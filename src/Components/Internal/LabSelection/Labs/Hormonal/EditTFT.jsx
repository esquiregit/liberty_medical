...
function EditTFT({ lab, patient, closeModal }) {
    ...
    const handleClose  = () => { setOpen(false); closeModal('tft'); };
    ...
    const onSubmit     = () => {
        ...
        Axios.post(getBaseURL()+'edit_tft', values, { signal: signal })
            .then(response => {
                if(response.data[0].status.toLowerCase() === 'success') {
                    setSuccess(true);
                    setMessage(response.data[0].message);
                    setTimeout(() => { closeModal('tft'); }, 1050);
                } else {
                    setError(true);
                    setMessage(response.data[0].message);
                }
