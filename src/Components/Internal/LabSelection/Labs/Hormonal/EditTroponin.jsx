...
function EditTroponin({ lab, patient, closeModal }) {
    ...
    const handleClose  = () => { setOpen(false); closeModal('troponin'); };
    ...
    const onSubmit     = () => {
        ...
        Axios.post(getBaseURL()+'edit_troponin', values, { signal: signal })
            .then(response => {
                if(response.data[0].status.toLowerCase() === 'success') {
                    setSuccess(true);
                    setMessage(response.data[0].message);
                    setTimeout(() => { closeModal('troponin'); }, 1050);
                } else {
                    setError(true);
                    setMessage(response.data[0].message);
                }
