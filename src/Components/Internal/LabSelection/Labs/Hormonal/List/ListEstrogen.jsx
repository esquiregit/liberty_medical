...
import ViewEstrogen from '../View/ViewEstrogen';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListEstrogen({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Estrogen | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_estrogen', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewEstrogen
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Estrogen" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Estrogen Lab" plural="Estrogen Labs" />
                }
            </main>
            <Footer />
        </>
