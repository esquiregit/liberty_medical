...
import ViewHormonalAssay from '../View/ViewHormonalAssay';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListHormonalAssay({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Hormonal Assay | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_hormonal_assay', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewHormonalAssay
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Hormonal Assay" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Hormonal Assay Lab" plural="Hormonal Assay Labs" />
                }
            </main>
            <Footer />
        </>
...
export default ListHormonalAssay;
