...
import ViewPSA from '../View/ViewPSA';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListPSA({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'PSA | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_psa', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewPSA
                                                        history={history}
                    ...
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ...
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="PSA Lab" plural="PSA Labs" />
                }
            </main>
            <Footer />
        </>
