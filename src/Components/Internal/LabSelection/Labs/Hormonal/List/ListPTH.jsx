...
import ViewPTH from '../View/ViewPTH';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListPTH({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'PTH | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_pth', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewPTH
                                                        history={history}
                    ...
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ...
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="PTH Lab" plural="PTH Labs" />
                }
            </main>
            <Footer />
        </>
