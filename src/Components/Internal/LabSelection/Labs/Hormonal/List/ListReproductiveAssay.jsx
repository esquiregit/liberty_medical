...
import ViewReproductiveAssay from '../View/ViewReproductiveAssay';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListReproductiveAssay({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Reproductive Assay | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_reproductive_assay', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewReproductiveAssay
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Reproductive Assay" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Reproductive Assay Lab" plural="Reproductive Assay Labs" />
                }
            </main>
            <Footer />
        </>
...
export default ListReproductiveAssay;
