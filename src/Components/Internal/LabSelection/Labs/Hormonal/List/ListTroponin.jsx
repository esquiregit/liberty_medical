...
import ViewTroponin from '../View/ViewTroponin';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListTroponin({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Troponin | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_troponin', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewTroponin
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Troponin" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Troponin Lab" plural="Troponin Labs" />
                }
            </main>
            <Footer />
        </>
