...
function AddHIVViralLoad({ patient, closeModal }) {
    ...
    const handleClose  = () => { setOpen(false); closeModal('HIVViralLoad'); };
    ...
    const onSubmit     = () => {
        ...
        Axios.post(getBaseURL()+'add_hiv_viral_load', values, { signal: signal })
            .then(response => {
                if(response.data[0].status.toLowerCase() === 'success') {
                    setSuccess(true);
                    setMessage(response.data[0].message);
                    setTimeout(() => { closeModal('HIVViralLoad'); }, 1050);
                } else {
                    setError(true);
                    setMessage(response.data[0].message);
                }
...
export default AddHIVViralLoad;
