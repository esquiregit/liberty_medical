...
function AddHepatitisMarkers({ patient, closeModal }) {
    ...
    const onSubmit     = () => {
        ...
        Axios.post(getBaseURL()+'add_hepatitis_markers', values, { signal: signal })
            .then(response => {
                if(response.data[0].status.toLowerCase() === 'success') {
                    setSuccess(true);
                    setMessage(response.data[0].message);
                    setTimeout(() => { closeModal('HepatitisMarkers'); }, 1050);
                } else {
                    setError(true);
                    setMessage(response.data[0].message);
                }
...
export default AddHepatitisMarkers;
