...
function EditHPyloriAg({ lab, closeModal, closeExpandable }) {
    ...
    const handleClose  = () => { setOpen(false); closeModal('HPyloriAg'); };
    ...
    const onSubmit     = () => {
        ...
        Axios.post(getBaseURL()+'edit_h_pylori_ag', values, { signal: signal })
            .then(response => {
                if(response.data[0].status.toLowerCase() === 'success') {
                    setSuccess(true);
                    setMessage(response.data[0].message);
                    setTimeout(() => { closeModal('HPyloriAg'); }, 1050);
                } else {
                    setError(true);
                    setMessage(response.data[0].message);
                }
