...
function EditHPyloriAgBlood({ lab, patient, closeModal }) {
    ...
    const handleClose  = () => { setOpen(false); closeModal('HPyloriAgBlood'); };
    ...
    const onSubmit     = () => {
        ...
        Axios.post(getBaseURL()+'edit_h_pylori_ag_blood', values, { signal: signal })
            .then(response => {
                if(response.data[0].status.toLowerCase() === 'success') {
                    setSuccess(true);
                    setMessage(response.data[0].message);
                    setTimeout(() => { closeModal('HPyloriAgBlood'); }, 1050);
                } else {
                    setError(true);
                    setMessage(response.data[0].message);
                }
...
export default EditHPyloriAgBlood;
