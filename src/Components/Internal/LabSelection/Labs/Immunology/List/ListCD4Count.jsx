...
import ViewCD4Count from '../View/ViewCD4Count';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListCD4Count({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'CD4 Count | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_cd4_count', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewCD4Count
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / CD4 Count" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="CD4 Count Lab" plural="CD4 Count Labs" />
                }
            </main>
            <Footer />
        </>
