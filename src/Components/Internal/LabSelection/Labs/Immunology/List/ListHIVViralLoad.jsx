...
import ViewHIVViralLoad from '../View/ViewHIVViralLoad';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListHIVViralLoad({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'HIV Viral Load | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_hiv_viral_load', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewHIVViralLoad
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / HIV Viral Load" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="HIV Viral Load Lab" plural="HIV Viral Load Labs" />
                }
            </main>
            <Footer />
        </>
...
export default ListHIVViralLoad;
