...
import ViewHPyloriAg from '../View/ViewHPyloriAg';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListHPyloriAg({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'H. Pylori Ag. | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_h_pylori_ag', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewHPyloriAg
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / H. Pylori Ag." />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="H. Pylori Ag. Lab" plural="H. Pylori Ag. Labs" />
                }
            </main>
            <Footer />
        </>
