...
import ViewHPyloriAgBlood from '../View/ViewHPyloriAgBlood';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListHPyloriAgBlood({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'H. Pylori Ag. |Blood Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_h_pylori_ag_blood', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewHPyloriAgBlood
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / H. Pylori Ag. Blood" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="H. Pylori Ag. LBlood" plural="H. Pylori Ag. LBlood" />
                }
            </main>
            <Footer />
        </>
...
export default ListHPyloriAgBlood;
