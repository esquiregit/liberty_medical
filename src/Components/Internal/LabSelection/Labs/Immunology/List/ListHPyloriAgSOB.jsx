...
import ViewHPyloriAgSOB from '../View/ViewHPyloriAgSOB';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListHPyloriAgSOB({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'H. Pylori Ag. |SOB Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_h_pylori_ag_sob', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewHPyloriAgSOB
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / H. Pylori Ag. SOB" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="H. Pylori Ag. SOB Lab" plural="H. Pylori Ag. SOB Labs" />
                }
            </main>
            <Footer />
        </>
...
export default ListHPyloriAgSOB;
