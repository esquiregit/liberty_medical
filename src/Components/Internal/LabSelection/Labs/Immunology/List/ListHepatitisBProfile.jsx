...
import ViewHepatitisBProfile from '../View/ViewHepatitisBProfile';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListHepatitisBProfile({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Hepatitis B Profile | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_hepatitis_b_profile', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewHepatitisBProfile
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Hepatitis B Profile" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Hepatitis B Profile Lab" plural="Hepatitis B Profile Labs" />
                }
            </main>
            <Footer />
        </>
...
export default ListHepatitisBProfile;
