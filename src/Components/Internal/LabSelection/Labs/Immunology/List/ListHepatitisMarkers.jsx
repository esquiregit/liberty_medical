...
import ViewHepatitisMarkers from '../View/ViewHepatitisMarkers';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListHepatitisMarkers({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Hepatitis Markers | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_hepatitis_markers', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewHepatitisMarkers
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Hepatitis Markers" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Hepatitis Markers Lab" plural="Hepatitis Markers Labs" />
                }
            </main>
            <Footer />
        </>
...
export default ListHepatitisMarkers;
