...
import ViewMantoux from '../View/ViewMantoux';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListMantoux({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Mantoux | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_mantoux', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const columns   = [
        ...
        {
            label: "Size of Induration in mm",
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewMantoux
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Mantoux" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Mantoux Lab" plural="Mantoux Labs" />
                }
            </main>
            <Footer />
        </>
