...
import ViewPregnancyTest from '../View/ViewPregnancyTest';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListPregnancyTest({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Pregnancy Test | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_pregnancy_test', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewPregnancyTest
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Pregnancy Test" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Pregnancy Test Lab" plural="Pregnancy Test Labs" />
                }
            </main>
            <Footer />
        </>
...
export default ListPregnancyTest;
