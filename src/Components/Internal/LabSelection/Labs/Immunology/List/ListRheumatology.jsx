...
import ViewRheumatology from '../View/ViewRhematology';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListRheumatology({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'Rheumatology | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_rheumatology', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewRheumatology
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / Rheumatology" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="Rheumatology Lab" plural="Rheumatology Labs" />
                }
            </main>
            <Footer />
        </>
...
export default ListRheumatology;
