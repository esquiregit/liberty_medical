...
function EditCA153({ lab, closeModal, closeExpandable }) {
    ...
    const handleClose  = () => { setOpen(false); closeModal('CA153'); };
    ...
    const onSubmit     = () => {
        ...
        Axios.post(getBaseURL()+'edit_ca_one_five_three', values, { signal: signal })
            .then(response => {
                if(response.data[0].status.toLowerCase() === 'success') {
                    setSuccess(true);
                    setMessage(response.data[0].message);
                    setTimeout(() => { closeModal('CA153'); }, 1050);
                } else {
                    setError(true);
                    setMessage(response.data[0].message);
                }
