...
function EditCRPUltraSensitive({ lab, closeModal, closeExpandable }) {
    ...
    const handleClose  = () => { setOpen(false); closeModal('CRPUltraSensitive'); };
    ...
    const onSubmit     = () => {
        ...
        Axios.post(getBaseURL()+'edit_crp_ultra_sensitive', values, { signal: signal })
            .then(response => {
                if(response.data[0].status.toLowerCase() === 'success') {
                    setSuccess(true);
                    setMessage(response.data[0].message);
                    setTimeout(() => { closeModal('CRPUltraSensitive'); }, 1050);
                } else {
                    setError(true);
                    setMessage(response.data[0].message);
                }
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td className="text-centre pt-20 pb-20" colSpan="5">
                                                -Cardiovascular risks, active ischemic heart disease, acute coronary syndromes (unstable angina)
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>Comments</td>
...
export default EditCRPUltraSensitive;
