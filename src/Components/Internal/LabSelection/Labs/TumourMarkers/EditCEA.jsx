...
function EditCEA({ lab, patient, closeModal }) {
    ...
    const handleClose  = () => { setOpen(false); closeModal('CEA'); };
    ...
    const onSubmit     = () => {
        ...
        Axios.post(getBaseURL()+'edit_cea', values, { signal: signal })
            .then(response => {
                if(response.data[0].status.toLowerCase() === 'success') {
                    setSuccess(true);
                    setMessage(response.data[0].message);
                    setTimeout(() => { closeModal('CEA'); }, 1050);
                } else {
                    setError(true);
                    setMessage(response.data[0].message);
                }
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td className="text-centre pt-20 pb-20" colSpan="5">
                                                Increase in: -Colon, stomach, pancreas, lung cancers (75%), breast, head and neck, ovarian cancern (50%), in 90% of all with solid-tissues tumours especially with metastases to liver or lung
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>Comments</td>
