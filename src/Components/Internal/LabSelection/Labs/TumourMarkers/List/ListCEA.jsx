...
import ViewCEA from '../View/ViewCEA';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListCEA({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'CEA | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_cea', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewCEA
                                                        history={history}
                    ...
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ...
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="CEA Lab" plural="CEA Labs" />
                }
            </main>
            <Footer />
        </>
