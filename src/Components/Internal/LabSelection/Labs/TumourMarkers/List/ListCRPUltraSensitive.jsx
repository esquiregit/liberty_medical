...
import ViewCRPUltraSensitive from '../View/ViewCRPUltraSensitive';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListCRPUltraSensitive({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'CRP Ultra Sensitive | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_crp_ultra_sensitive', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewCRPUltraSensitive
                                                        history={history}
    ...
    return (
        <>
            ...
            <main
                ...
                <Breadcrumb page="History / CRP Ultra Sensitive" />
                {
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ?
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="CRP Ultra Sensitive Lab" plural="CRP Ultra Sensitive Labs" />
                }
            </main>
            <Footer />
        </>
...
export default ListCRPUltraSensitive;
