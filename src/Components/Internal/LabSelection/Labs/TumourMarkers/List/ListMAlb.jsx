...
import ViewMAlb from '../View/ViewMAlb';
import { getBaseURL } from '../../../../../Extras/server';
import { useSelector } from 'react-redux';

function ListMAlb({ history }) {
    ...
    React.useEffect(()    => {
        document.title        = 'M-Alb | Liberty Medical Labs';
        const abortController = new AbortController();
        const signal          = abortController.signal;
        
        if(staff) {
            if(permissions && (permissions.includes("Can View Lab List") || permissions.includes("Can Edit Lab"))) {
                Axios.post(getBaseURL()+'get_m_alb', { role: staff.role_name, branch: staff.branch }, { signal: signal })
                    .then(response => {
    ...
    const options = {
        ...
        renderExpandableRow: (rowData, rowMeta) => <ViewMAlb
                                                        history={history}
                    ...
                    loading ? <Loader /> :
                        (labs && labs.length)
                            ...
                            <MUIDataTable
                                ...
                                options={options} />
                            :
                            <EmptyData error={comError} single="M-Alb Lab" plural="M-Alb Labs" />
                }
            </main>
            <Footer />
        </>
