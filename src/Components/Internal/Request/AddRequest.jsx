...
import { getRequestData, getDefaultRequestValues } from '../../Extras/Functions';
import { DialogContent, DialogActions, DialogTitle, Transition } from '../../Extras/Dialogue';
import 'tippy.js/dist/tippy.css';

function AddRequest({ patient, closeAddRequestModal, closeExpandable, staff_id }) {
    const classes = styles();

    const [values, setValues]     = useState(getDefaultRequestValues());
    ...
    const handleClose    = () => { setOpen(false); closeAddRequestModal(); };
    ...
    const onSubmit       = () => {
        ...
        Axios.post(getBaseURL()+'add_request', data, { signal: signal })
            .then(response => {
                if(response.data[0].status.toLowerCase() === 'success') {
                    let request   = {
                        ...
                        request_id      : response.data[0].request[0].request_id     ,
                        total_cost      : response.data[0].request[0].total_cost     ,
                        discounted_cost : response.data[0].request[0].discounted_cost,
                    };
                    setOpen(false);
                    closeExpandable(response.data[0].message, null, null, request);
    ...
    const reset          = () => {
        setFormValid(false);
        setValues(getDefaultRequestValues());
    }
    ...
    return (
        <>
            ...
            { showDialogue && <ConfirmDialogue message={'Are You Sure You Want To Add Request?'} closeConfirm={closeConfirm} /> }
            <Backdrop className={classes.backdrop} open={backdrop}>
                <CircularProgress color="inherit" /> <span className='ml-15'>Adding Request. Please Wait....</span>
            </Backdrop>
            <Dialog
                open={open}
                onClose={handleClose}
                ...
                <DialogTitle id="customized-dialog-title" onClose={handleClose}>
                    Add Request For {patient.first_name} {patient.middle_name} {patient.last_name}
                </DialogTitle>
                            ...
                            <FormGroup>
                                ...
                                <Grid container spacing={3}>
                                    <Grid item xs={12} sm={3}>
                                        <FormControlLabel
                                            ...
                                            label="24hr ogtt" />
                                        <FormControlLabel
                                            control={<Checkbox checked={values[0] ? values[0].twenty_four_post_prandial : false} onChange={handleChange} name="twenty_four_post_prandial" />}
                                            label="24hr post prandial" />
                                        <FormControlLabel
                                            control={<Checkbox checked={values[0] ? values[0].twenty_four_urine_protein : false} onChange={handleChange} name="twenty_four_urine_protein" />}
                                            label="24hr urine protein" />
                                        <FormControlLabel
                                            control={<Checkbox checked={values[0] ? values[0].twenty_four_vma : false} onChange={handleChange} name="twenty_four_vma" />}
                                            label="24hr vma" />
                                        <FormControlLabel
                                ...
                                <Grid container spacing={3}>
                                    <Grid item xs={12} sm={4}>
                                        ...
                                        <FormControlLabel
                                            control={<Checkbox checked={values[0] ? values[0].twenty_four_urine_cortisol : false} onChange={handleChange} name="twenty_four_urine_cortisol" />}
